...
# Output directory
OUTPUT_DIR = Path('mvp-openalex/mvp_output')
...
class MVPExtractor:
    ...
    def execute_sql_file(self, sql_file, output_csv=None):
        ...
        try:
            ...
            # The extraction query should be the largest SELECT result
            if output_csv:
                if df is not None and len(df) > 0:
                    output_path = OUTPUT_DIR / output_csv
                    df.to_csv(output_path, index=False)
                    logger.info(f"✓ Saved: {output_csv} ({len(df):,} rows, {len(df.columns)} columns)")
                    logger.info(f"✓ File path: {output_path}")
                    return df
                else:
                    logger.warning(f"⚠ No data to save for {output_csv}")
                    logger.warning(f"   Last df had {len(df) if df is not None else 0} rows")
                    return None
    ...
    def run_full_pipeline(self):
        ...
        steps = [
            ("01_Works", 
             Path("mvp-openalex/mvp_extract_01_works.sql"), 
             "works.csv"),
            
            ("02_Authors", 
             Path("mvp-openalex/mvp_extract_02_authors.sql"), 
             "authors.csv"),
            
            ("03_Authorships", 
             Path("mvp-openalex/mvp_extract_03_authorships.sql"), 
             "authorships.csv"),
            
            ("04_Institutions", 
             Path("mvp-openalex/mvp_extract_04_institutions.sql"), 
             "institutions.csv"),
            
            ("05_Topics", 
             Path("mvp-openalex/mvp_extract_05_topics.sql"), 
             "topics.csv"),
            
            ("06_Citations", 
             Path("mvp-openalex/mvp_extract_06_citations.sql"), 
             "citations.csv"),
            
            ("07_External_Works", 
             Path("mvp-openalex/mvp_extract_07_external_works.sql"), 
             "external_works.csv"),
            
            ("08_Sources", 
             Path("mvp-openalex/mvp_extract_08_sources_publishers.sql"), 
             "sources.csv"),
            
            ("09_Related_Works", 
             Path("mvp-openalex/mvp_extract_09_related_works.sql"), 
             "related_works.csv"),
        ]
